...
class music(object):
    ...
    def save_music(self, artist_id):
        ...
        # 获取歌手个人主页
        r = requests.get('http://music.163.com/artist/', headers=self.headers, params=params)

        # 网页解析
        soup = BeautifulSoup(r.content.decode(), 'html.parser')
        body = soup.body

        musics = body.find('ul', attrs={'class': 'f-hide'}).find_all('li')   # 获取所有热门歌曲

        for music in musics:
            music = music.find('a')
            music_id = music['href'].replace('/song?id=', '')
            music_name = music.getText()
            sql.insert_music(music_id, music_name)
